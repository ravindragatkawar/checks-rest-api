...
export function sendSMS(mobileNo,message,callback) {
    const accountSid = process.env.TWILIO_ACCOUNT_SID;
    const authToken  = process.env.TWILIO_AUTH_TOKEN;

    const payload = {
        'To'   : mobileNo,
        'From' : config.fromMobileNo,
        'Body' : message 
    }
    ...
    //----- bind request to error event , so it won't get thrown -----
    req.on('error',(err) => {
        console.error(err);
        callback(err);  
    })
    ...
    //---- end the reqest, here request is sent -----
    req.end();
}
