    ...
    storage.read('users',cpObj.mobileNo.toString(),(err,userObj) => {
        if (err) {
            ...
            if (cpObj.mobileNo === userObj.mobileNo && cpObj.password === userObj.password) {
                ...
                userObj.tokenExpiry = Date.now() + 1000*60*60;
...
token.DELETE = (routeData,response) => {
    ...
    storage.read('tokens',cpObj.tokenId,(err,tokenObj) => {
        if (err) {
            console.error(err);
            response(404,{'err':1, 'msg':'Token Not Found'});
        }
        else {
            if (cpObj.mobileNo === tokenObj.mobileNo) {
                storage.delete('tokens',tokenObj.tokenId,(err) => {
                    if (err) {
                        ...
                        response(200,{'err':0, 'msg':'Token Deleted SuccessFully'});
                    }
                });
            }
            else {
                response(409,{'err':1, 'msg':'Invalid TokenId/MobileNo'})
            }
        }
    });
