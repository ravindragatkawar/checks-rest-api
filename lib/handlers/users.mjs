...
import errCode from '../errorcodes.mjs';
...
user.POST = (routeData,response) => {
    ...
    //----- if generated tokenId exits, repond with error to retry ----
    let tokenObj = {};
    ...
    storage.create('tokens',tokenObj.tokenId,tokenObj,(err) => {
        if (err) {
            // random generated tokenId collision, tokenId associated with existing user -----
            console.log(err)
            response(500,'')
        }
    });
