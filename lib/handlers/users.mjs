...
user.PUT = (routeData,response) => {
    ...
    storage.read('users',cpObj.mobileNo.toString(),(err,userObj) => {
        if (err) {
            console.error(err);
            response(404,{'err':1, 'msg':'User Not Found'});
        }
        else {
            if (cpObj.tokenId === userObj.tokenId) {
                if (userObj.tokenExpiry < Date.now()) {
                ...
                if (cpObj.newMobileNo) {
                    ...
                    storage.create('users',userObj.mobileNo.toString(),userObj,(err) => {
                        if (err) {
                            console.error(err);
                            response(409,{'err':1, 'msg':'User Already Exists With Requested newMobileNo'});
                        }
                        else {
                            ...
                            //----- update mobileNo in token file -----
                            const tokenObj = {
                                'mobileNo' : userObj.mobileNo,
                                'tokenId' : userObj.tokenId,
                                'tokenExpiry' : userObj.tokenExpiry
                            };
                            storage.update('tokens',userObj.tokenId,tokenObj,(err) =>{
